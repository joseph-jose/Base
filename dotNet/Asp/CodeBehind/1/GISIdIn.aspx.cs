...
public partial class GISIdIn : System.Web.UI.Page
{
        ...
        string vSqlText = "";
            switch (inKey.ToUpper())
            {
                case "EQUIPMENTID":
                    ...
                    vSqlText = "SELECT DISTINCT [gis_id] , ISNULL([equip_id], 0)  [equip_id], ISNULL([compkey] , 0) [compkey]  ,[status]  ,[fclass]  ,Layer as [NetViewLayer] FROM [GISNet2].[GISWSL].[WA_Netview_View] WHERE [status] <> 'REMOVED' and ([EQUIP_ID] = '{0}') ORDER BY [fclass] DESC";
                ...
                case "COMPKEY":
                    ...
                    vSqlText = "SELECT DISTINCT [gis_id] , ISNULL([equip_id], 0)  [equip_id], ISNULL([compkey] , 0) [compkey] ,[status]  ,[fclass]  ,Layer as [NetViewLayer] FROM [GISNet2].[GISWSL].[WA_Netview_View] WHERE ([status] <> 'RM') and ([compkey] = '{0}') ORDER BY [fclass] DESC";
                    vSqlText = String.Format(vSqlText, inValue);
                    break;
                case "GISIDNET1":
                    ...
                    vSqlText = "SELECT DISTINCT [gis_id] , ISNULL([equip_id], 0)  [equip_id], ISNULL([compkey] , 0) [compkey] ,[status]  ,[fclass]  ,Layer as [NetViewLayer], 'SAP' as Source FROM [GISNet2].[GISWSL].[WA_Netview_View] WHERE [status] <> 'REMOVED' and ([GIS_ID] = '{0}') ";
                ...
                case "GISIDNET2":
                    ...
                    vSqlText = "SELECT DISTINCT [gis_id] , ISNULL([equip_id], 0)  [equip_id], ISNULL([compkey] , 0) [compkey] ,[status]  ,[fclass]  ,Layer as [NetViewLayer], 'HANSEN' as Source FROM [GISNet2].[GISWSL].[WA_Netview_View] WHERE ([status] <> 'RM') and ([GIS_ID] = '{0}') ";
                    vSqlText = vSqlText + "ORDER BY [fclass] DESC";
                    vSqlText = String.Format(vSqlText, inValue, inValue);
                    break;
            }
    ...
    private bool buildSAPKeyUrls(DataRow inArr , out string outStr) 
    {
        ...
        //outStr = "http://watercare.maps.arcgis.com/apps/webappviewer/index.html?id=405ab83e0abf406e837e4185b89646d5&query={0},GIS_ID={1}";
        //outStr = "http://watercare.maps.arcgis.com/apps/webappviewer/index.html?id=633276634e64476db32bf43fa9349c6b&query={0},GIS_ID={1}";
        outStr = "http://watercare.maps.arcgis.com/apps/webappviewer/index.html?id=e46bdd5f7dd54bbd9d09cae93f208159&query={0},GIS_ID={1}";
